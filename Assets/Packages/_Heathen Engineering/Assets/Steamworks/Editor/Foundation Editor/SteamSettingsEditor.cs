        ...
        private void DrawAchievementList()
        {
            ...
            if (achievements)
            {
                ...
                if (GUILayout.Button("Import", EditorStyles.toolbarButton, GUILayout.Width(50)))
                {
                    try
                    {
                        ...
                        for (int i = 0; i < settings.achievements.Count; i++)
                        {
                            ...
                            if (!names.Contains(achievement.achievementId))
                            {
                        ...
                        for (int i = 0; i < names.Length; i++)
                        {
                            ...
                            var achObj = settings.achievements.FirstOrDefault(p => p.achievementId == achName);
                            ...
                            achObj.achievementId = achName;
                            achObj.displayName = API.StatsAndAchievements.Client.GetAchievementDisplayAttribute(achName, AchievementAttributes.name);
                            achObj.displayDescription = API.StatsAndAchievements.Client.GetAchievementDisplayAttribute(achName, AchievementAttributes.desc);
                ...
                for (int i = 0; i < settings.achievements.Count; i++)
                {
                    ...
                    EditorGUILayout.LabelField(target.achievementId);
...
#region Steam Complete
            ...
            if (dlcFoldout)
            {
                ...
                if (GUILayout.Button("Import", EditorStyles.toolbarButton, GUILayout.Width(50)))
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (!dlc.Any(p => p.AppId == eDlc.appId))
                                toRemove.Add(eDlc);
                        }
        ...
        private void DrawInventoryArea()
        {
            ...
            if (inventoryFoldout)
            {
                ...
                if (GUILayout.Button("+ New", EditorStyles.toolbarButton, GUILayout.Width(50)))
                {
                    ...
                    nItem.item_itemdefid = GetNextAvailableItemNumber();
                    nItem.item_name.value = "New Item";
                    nItem.name = "[Inv] " + nItem.item_itemdefid + " New Item";
                ...
                itemsFoldout = EditorGUILayout.Foldout(itemsFoldout, "Items: " + settings.client.inventory.items.Where(p => p.item_type == Enums.InventoryItemType.item).Count());

                if (itemsFoldout)
                {
                    settings.client.inventory.items.Sort((a, b) => a.item_itemdefid.CompareTo(b.item_itemdefid));

                    for (int i = 0; i < settings.client.inventory.items.Count; i++)
                    {
                        var item = settings.client.inventory.items[i];

                        if (item.item_type == Enums.InventoryItemType.item)
                        {
                            if (DrawItem(item))
                                break;
                        }
                ...
                bundlesFoldout = EditorGUILayout.Foldout(bundlesFoldout, "Bundles: " + settings.client.inventory.items.Where(p => p.item_type == Enums.InventoryItemType.bundle).Count());
                ...
                if (bundlesFoldout)
                {
                    ...
                    for (int i = 0; i < settings.client.inventory.items.Count; i++)
                    {
                        ...
                        if (item.item_type == Enums.InventoryItemType.bundle)
                        {
                            if (DrawItem(item))
                                break;
                        }
                ...
                generatorFoldout = EditorGUILayout.Foldout(generatorFoldout, "Generators: " + settings.client.inventory.items.Where(p => p.item_type == Enums.InventoryItemType.generator).Count());
                ...
                if (generatorFoldout)
                {
                    ...
                    for (int i = 0; i < settings.client.inventory.items.Count; i++)
                    {
                        ...
                        if (item.item_type == Enums.InventoryItemType.generator)
                        {
                            if (DrawItem(item))
                                break;
                        }
                ...
                playtimegeneratorFoldout = EditorGUILayout.Foldout(playtimegeneratorFoldout, "Playtime Generators: " + settings.client.inventory.items.Where(p => p.item_type == Enums.InventoryItemType.playtimegenerator).Count());
                ...
                if (playtimegeneratorFoldout)
                {
                    ...
                    for (int i = 0; i < settings.client.inventory.items.Count; i++)
                    {
                        ...
                        if (item.item_type == Enums.InventoryItemType.playtimegenerator)
                        {
                            if (DrawItem(item))
                                break;
                        }
                ...
                taggeneratorFoldout = EditorGUILayout.Foldout(taggeneratorFoldout, "Tag Generators: " + settings.client.inventory.items.Where(p => p.item_type == Enums.InventoryItemType.tag_generator).Count());
        ...
        {
            int id = 1;
            while (settings.client.inventory.items.Any(p => p.item_itemdefid == id) && id < 999999)
                id++;
